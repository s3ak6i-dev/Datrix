...
import {
  ...
  Users, GitPullRequest, Bell, RotateCcw, MessageSquare,
} from 'lucide-react'
...
const STEPS: StepDef[] = [
  ...
  },
  {
    ...
    iconBg: 'rgba(99,179,255,0.12)',
    iconColor: 'var(--accent)',
    tagline: 'Invite your team, assign roles, and collaborate with full access control',
    flow: [
      { label: 'Create a workspace', detail: 'During onboarding, name your organization — every dataset, pipeline, and change request lives inside it' },
      { label: 'Invite teammates', detail: 'Go to Settings → Members, copy the unique invite link, and share it. New members pick a display name and color on join.' },
      { label: 'Assign roles', detail: 'Each member is an owner, reviewer, or member — roles control who can approve, reject, and roll back change requests' },
      { label: 'Track team activity', detail: 'The Home page activity feed shows every approved and rolled-back change across the workspace with submitter and timestamp' },
    ],
    caps: [
      { icon: Shield, title: 'Owner', desc: 'Full access: approve / reject / rollback any CR, manage all members, regenerate invite links, and configure workspace settings.' },
      { icon: Eye, title: 'Reviewer', desc: 'Can approve and reject change requests. Can roll back approvals they personally gave — not other reviewers\' decisions.' },
      { icon: Lock, title: 'Member', desc: 'Can submit change requests and view all workspace data. No approval powers — changes enter a pending review queue.' },
      { icon: Users, title: 'Invite links', desc: 'Each workspace has a unique join URL. Regenerating it in Settings instantly invalidates the old one for security.' },
      { icon: Sparkles, title: 'Member colors', desc: 'Each member picks a color on signup — used as their avatar indicator on CR cards, activity rows, and notifications.' },
      { icon: List, title: 'Activity feed', desc: 'Home page shows the 20 most recent approved and rolled-back CRs — click any row to jump directly to that CR on the board.' },
      { icon: Settings, title: 'Member management', desc: 'Owners can promote, demote, or remove members at any time. Role changes take effect immediately across the workspace.' },
      { icon: Network, title: 'Multi-workspace', desc: 'You can belong to multiple workspaces. The org selector in the top bar switches context instantly without re-logging in.' },
    ],
    route: '/settings',
  },
  {
    ...
    iconBg: 'rgba(192,132,252,0.12)',
    iconColor: '#c084fc',
    tagline: 'Every change tracked, reviewed, and auditable — with comments, rollbacks, and instant notifications',
    flow: [
      { label: 'Submit a CR', detail: 'Click "Request Change" and fill in title, description, and impact level (low / medium / high / critical)' },
      { label: 'Reviewers get notified', detail: 'Owners and reviewers receive an instant in-app notification with a link directly to the pending card on the Kanban board' },
      { label: 'Approve or reject', detail: 'Reviewer opens the card, leaves an optional comment, and clicks Approve or Reject — submitter is notified of the decision' },
      { label: 'Rollback if needed', detail: 'Any approved CR can be rolled back with a reason. The submitter is notified and can resubmit the CR once issues are resolved.' },
    ],
    caps: [
      { icon: GitPullRequest, title: 'Kanban board', desc: '4 columns: Pending, Approved, Rejected, Rolled Back. Filter by org, search by title — each card shows submitter, impact, and reviewer.' },
      { icon: MessageSquare, title: 'Approval comments', desc: 'Reviewers can attach a comment when approving or rejecting. The comment is shown directly on the CR card for everyone.' },
      { icon: RotateCcw, title: 'Rollback', desc: 'Owners can roll back any approved CR. Reviewers can only roll back approvals they personally gave. A reason is recorded on the card.' },
      { icon: Zap, title: 'Auto-approve', desc: 'Low and medium-impact CRs automatically approve after 24 hours if no reviewer acts — preventing low-stakes changes from stalling.' },
      { icon: Bell, title: 'Real-time notifications', desc: 'Bell icon in the nav shows unread count. Reviewers notified of new CRs; submitters notified on approve / reject / rollback.' },
      { icon: Target, title: 'Deep-link navigation', desc: 'Clicking a CR notification takes you directly to that card on the board — it scrolls into view and pulses briefly to orient you.' },
      { icon: FileText, title: 'Resubmit flow', desc: 'Rejected or rolled-back CRs can be resubmitted after making changes. A counter tracks how many times a CR has been resubmitted.' },
      { icon: BarChart3, title: 'Job notifications', desc: 'Synthetic, Active Learning, and Benchmark jobs also fire notifications on completion or failure — same bell, same click-to-navigate.' },
    ],
    note: 'Notifications are stored locally in localStorage (up to 50). They persist across page refreshes but are per-browser — other team members receive their own independent notification streams.',
    route: '/changes',
  },
  {
    id: 'ready',
    ...
    caps: [
      ...
      { icon: GitPullRequest, title: 'Change Requests', desc: 'Submit, review, approve, and roll back changes — with comments and instant notifications.', route: '/changes' },
      { icon: Users, title: 'Workspaces', desc: 'Invite teammates, assign owner / reviewer / member roles, and manage your org.', route: '/settings' },
